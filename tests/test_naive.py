...
class TestNaive(unittest.TestCase):
    ...
    def test_directed(self):
        ...
        self.assertEqual(all_partial_hausdorff(A, B), [10, 0])
        self.assertEqual(all_partial_hausdorff(B, A), [0])

    def test_identical_sets_give_zeros(self):
        A = line([1, 5, 9])
        self.assertEqual(all_partial_hausdorff(A, A), [0, 0, 0])
    ...
    def test_k_out_of_range(self):
        with self.assertRaises(IndexError):
            partial_hausdorff(line([1, 2]), line([1]), 2)
